...
function Program()
	ret = @glcall( (:glCreateProgram, lib), Program, ())
	if ret == 0
		GetError()
...
function AttachShader(p::Program, s::Shader)
	@glcall( (:glAttachShader, lib), Void, (Program, Shader), p, s)
	GetError()
end

function LinkProgram(p::Program)
	@glcall( (:glLinkProgram, lib), Void, (Program,), p)
...
function UseProgram(p::Program)
	@glcall( (:glUseProgram, lib), Void, (Program,), p)
...
function GetProgram(p::Program, param::Integer)
	ret = GLint[-1]
	@glcall( (:glGetProgramiv, lib), Void,
...
function GetProgramInfoLog(p::Program)
	...
	@glcall( (:glGetProgramInfoLog, lib), Void,
